...
/// @brief		Intantized the board, sets all the places to a space
void initBoard();

/// @brief		Updates the board with a 'X' or 'O'
...
/// @brief		Checks if the choice is valid or not. Valid means the space is empty. (No 'X' or 'O' char there)	
/// @param			int: choice to check
/// @return			bool: is it a valid choice or not
bool checkValidChoice(int choice);
/// @brief		Checks to see if the board is filled up with 'X' or 'O'	or not
/// @return			bool: if the board is filled or not
bool boardFilled();

/// @brief		Cleans up board so we can play again	
void cleanUpBoard();
